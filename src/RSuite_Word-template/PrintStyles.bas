...
Private Sub PrintStylesMac()
  ...
  ' True means a check failed (e.g., doc protection on)
    ...
    ' ===== Status Bar ========================
    If WT_Settings.showStatus = True Then
    ...
    'many styles triggers a warning and the default behavior is to paste as Normal style. Delay 1 second to allow the message to pop up first.
     If IsOldMac = True Then MacScript ("delay 1" & vbCr & "tell application " & Chr(34) & "System Events" & Chr(34) & " to key code 45")
...
'    strStatus = "* Getting started..."
'
'    Dim objProgressPrint As Progress_Bar
'    Set objProgressPrint = New Progress_Bar  ' Triggers Initialize event, which uses Show methond
...
'    Call UpdateBarAndWait(Bar:=objProgressPrint, Status:=strStatus, Percent:=sglPercentComplete)
    ...
    strPath = WT_Settings.StyleDir & Application.PathSeparator & "RSuite_Word-template.dotm"
    If IsItThere(strPath) = True Then
        Set objTemplate = Templates(strPath)
    Else
        MsgBox "I can't find the RSuite template, sorry."
        GoTo FinishUp
    End If
