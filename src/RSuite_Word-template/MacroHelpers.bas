...
Public Sub WriteToLog(LogMessage As String, Optional LogFilePath As String)
    ...
    If LogFilePath = vbNullString Then
        strLogFile = "C:" & Application.PathSeparator & _
...
Sub StyleAllHyperlinks(Optional ByVal StoriesInUse As Collection)
    ...
    With Selection.Find
      ...
      ' Check previous paragraph for Page Break
        ...
        If lngSectionStyle > 1 Then
          ...
          If activeDoc.Paragraphs(lngPrevSibling).Range.Style = WT_StyleConfig.MacmillanStyles("pagebreak") Then
...
' Remove any first paragraphs until we get one with text
  ...
  Do
    ...
    If MacroHelpers.IsNewLine(rngPara1.Text) = True And _
      WT_StyleConfig.IsSectionStartStyle(rngPara1.Style) = False Then
